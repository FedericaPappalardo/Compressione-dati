...
def get_landmarks(im):
    ...
    mylist=[]
    for myface in rects:
        #print(numpy.matrix([[p.x, p.y] for p in predictor(im, myface).parts()]))
        mylist.append(numpy.matrix([[p.x, p.y] for p in predictor(im, myface).parts()]))
...
for filename in os.listdir(input_folder):
    ...
    print(image_path)
    name_list = Riconosci_Volto(image_path)
    im1, landmarks = read_im_and_landmarks(image_path)
    
    #im2, landmarks3 = read_im_and_landmarks("tsunoda.png")
    i=0
    for x in landmarks:
        landmarks1=x
        if name_list[i]=="Joe Biden":
            volto_sost = "/Users/fede/Downloads/foto/coreano.jpg"
        if name_list[i]=="Barack Obama":
            volto_sost = "/Users/fede/Downloads/uomo.jpg"
        print(volto_sost)
        im2, landmarks3 = read_im_and_landmarks(volto_sost)
        for y in landmarks3:
            landmarks2=y
